...
from models import ResNet34, MetaNet, HeadsNet
...
parser.add_argument('--corruption_level', default=0.2, type=float, help='Corruption level')
parser.add_argument('--gold_fraction', default='0.02', type=float, help='Gold fraction')

parser.add_argument("--data-path", type=str, default=".")
parser.add_argument("--log-dir", type=str, default="outputs/cifar10_unif203/")
parser.add_argument('--meta_pretrained', type=str, default="")
parser.add_argument('--pretrained', type=str, default="sim_clr_cifar10.pth")
...
val_dataloader, noisy_dataloader, test_dataloader, num_classes = prepare_data(args.gold_fraction, args.corruption_level, args)

net = ResNet34(num_classes).to(args.device)
meta_net = MetaNet(features_size=512, embed_dim=128, out_size=num_classes).to(args.device)
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.max_epoch * len(noisy_dataloader), eta_min=1e-6)
...
for epoch in range(args.max_epoch):
    meta_train_one_epoch(net, meta_net, optimizer, meta_optimizer, meta_step, noisy_dataloader, val_dataloader,
                        scheduler, meta_scheduler, epoch, args.device, writer, num_classes=num_classes)
